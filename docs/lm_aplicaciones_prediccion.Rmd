...
Y el valor medio de $y_p$ es:

$$
E[y_p] = E[x_p^T \beta + u_p] = x_p^T \beta
$$

Por tanto la predicción del valor medio de $y_p$, $E[y_p]$, será:
...
# Varianza de la predicción del valor medio 
...
$$
\hat y_p - t_{\alpha/2} se(\hat y_p) \leq (x_p^T \beta) \leq \hat y_p + t_{\alpha/2} se(\hat y_p)
$$
...
También podemos decir algo acerca de la predicción de $y_p$, y no solo de su valor medio. Se define el error de predicción como la diferencia entre $y_p$ y su valor medio predicho $\hat{y}_p$:
...
Por tanto podemos calcular la distribución de $\epsilon_p = y_p - \hat y_p$:
...
$$
E[\epsilon_p] = E[y_p] - E[\hat y_p] = x_p^T \beta - x_p^T \beta = 0
$$

$$
Var[\epsilon_p] = Var[y_p] + Var[\hat y_p] = \sigma^2 + \sigma^2 v_p= \sigma^2 (1 + v_p)
$$

donde se ha considerado que $y_p$ e $\hat y_p$ son independientes. Utilizando la varianza residual tenemos:

$$
\frac{\epsilon_p}{\hat s_R\sqrt{1 + v_p}} \sim t_{n-k-1}
$$

Con lo que se puede encontrar que:

$$
P \left( - t_{\alpha/2} \hat s_R\sqrt{1 + v_p} \leq \epsilon_p \leq t_{\alpha/2} \hat s_R\sqrt{1 + v_p} \right) = 1 - \alpha
$$
...
- Si queremos asignar un *valor puntual* para la predicción de $y_p$ utilizaremos el valor medio predicho, $\hat y_p = x_p^T \hat \beta$, que es un estimador centrado del valor medio teórico, $x_p^T \beta$. Es posible calcular un intervalo de confianza para el valor medio teórico.
...
En este caso no se puede utilizar bootstrap. El método bootstrap se utiliza para calcular intervalos de confianza y se basa en remuestrear residuos. Pero $y_p$ es una variable aleatoria, no unos residuos. 