...
$$
\hat{\beta}^* \rightarrow N(\beta^*, \sigma^2 \tilde Q)
$$

donde

$$
\tilde Q = \frac{1}{n-1}(\tilde X^T \tilde X)^{-1}
$$ 

Esto implica que:

$$
\hat \beta_i \rightarrow N(\beta_i, \sigma^2 \tilde{Q}_{i,i}), \quad i = 1,2, \ldots,k
$$

donde $\tilde{Q}_{i,j}$ es el elemento *(i,j)* de la matriz $\tilde Q$. Por tanto, siguiendo el razonamiento del apartado anterior:

$$
se(\hat \beta_i) = \sqrt{ \hat s_R^2 \hat{Q}_{i,i}}, \quad i = 1,2,\cdots,k
$$
