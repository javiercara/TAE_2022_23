...
$$VT = \sum (y_i - \bar y)^2 = (n-1)\hat s_y^2$$

- Variabilidad explicada: 

$$VE = \sum (\hat y_i - \bar y)^2$$

- Variabilidad no explicada o residual: 

$$VNE = \sum e_i^2 = (n-k-1)\hat s_R^2$$
...
- Si $\beta_1 = \beta_2 = \cdots = \beta_k = 0 \Rightarrow VE/\sigma^2 \sim \chi^2_k$
- $VNE/\sigma^2 \sim \chi^2_{n-k-1}$
...
Y consideremos otro modelo de regresión en el que se utilizan *m* de los *k* regresores:
...
$$
\hat \beta_2 \sim N(\beta_2, \sigma^2 \tilde Q_{3,3}), \quad \hat \beta_3 \sim N(\beta_3, \sigma^2 \tilde Q_{4,4})
$$

Por tanto:

$$
\hat \beta_2 - \hat \beta_3 \sim N(\beta_2 - \beta_3, \sigma^2 (\tilde Q_{3,3} + \tilde Q_{4,4} - 2\tilde Q_{3,4}))
$$

Es decir:

$$
\frac{(\hat \beta_2 - \hat \beta_3) - (\beta_2 - \beta_3)}{\sqrt{\sigma^2 (\tilde Q_{3,3} + \tilde Q_{4,4} - 2\tilde Q_{3,4}))}} \sim N(0,1)
$$

Reemplazando $\sigma^2$ por $\hat s_R^2$ se tiene:

$$
\frac{(\hat \beta_2 - \hat \beta_3) - (\beta_2 - \beta_3)}{\sqrt{\hat s_R^2 (\tilde Q_{3,3} + \tilde Q_{4,4} - 2\tilde Q_{3,4}))}} \sim t_{n-k-1}
$$

Finalmente el estadístico del contraste cuando $H_0$ es cierta es:

$$
t_0 = \frac{\hat \beta_2 - \hat \beta_3}{\sqrt{\hat s_R^2 (\tilde Q_{3,3} + \tilde Q_{4,4} - 2\tilde Q_{3,4}))}} \sim t_{n-k-1}
$$
