...
También se conoce como **modelo logit**, ya que la función logit se define como:
...
$$
\beta_0 + \beta_1 x_i = log \left( \frac{\pi_i}{1-\pi_i} \right) = logit(\pi_i)
$$
...
Se considera que el máximo de $f(x)$ se alcanza cerca del máximo de la aproximación, es decir, cerca de $x_1$. Por eso se utiliza un polinomio de segundo grado, porque $x_1$ se calcula mediante una expresión analitica. Pero en general esto no tiene por qué ser cierto. Lo que se hace es volver a aproximar $f(x)$ por un polinomio de orden 2 en el entorno de $x_2$ y obtener el máximo de la nueva aproximación:
...
donde $x = [x_1 \ x_2 \ \cdots \ x_n]^T$, $G_k$ es el vector gradiente de *f* calculado en $x_k$, y $H_k$ es la matriz hessiana de *f* calculada en $x_k$. Por tanto, el algoritmo de Newton en caso de funciones multivariantes es:
